...
if run:
    if not TRAIN_YEARS:
        st.error("학습 연도를 1개 이상 선택하세요."); st.stop()
    if not methods:
    ...
    for industry, row in pv.iterrows():
        ...
        for m in methods:
            if m == "선형추세(OLS)":
                preds, _ = _linear_forecast(x, y, FORECAST_YEARS)
            elif m == "CAGR":
                preds, _ = _cagr_forecast(x, y, FORECAST_YEARS)
            else:  # Holt
                preds, _ = _holt_forecast(y, last_train, FORECAST_YEARS)
    ...
    st.subheader(f"업종별 예측 표 — 학습연도 {TRAIN_YEARS[0]}–{TRAIN_YEARS[-1]} / 예측연도 {FORECAST_YEARS[0]}–{FORECAST_YEARS[-1]}")
    ...
    def _add_method_sheet(method_label: str):
        ...
        for col in pred_cols: dfm[col] = final_sorted[col]
