...
with st.sidebar:
    ...
    # 리포에 올려둔 샘플 파일명을 기본값으로 사용
    sample_path = Path("산업용_업종별.xlsx")  # 필요 시 'sample_industry_usage.xlsx' 등으로 변경
...
def read_excel_to_long(file) -> pd.DataFrame:
    ...
    m = m[m["연도"].isin(YEARS)].rename(columns={cat_col: "업종"})
    m["사용량"] = pd.to_numeric(m["사용량"], errors="coerce")
    m = m.dropna(subset=["업종", "연도", "사용량"])
    return m[["업종", "연도", "사용량"]]

def _linear_forecast(x, y, target):
    coef = np.polyfit(x, y, 1);  yhat = np.polyval(coef, target);  fit = np.polyval(coef, x)
    return yhat, fit

def _poly_forecast(x, y, target, deg=3):
    deg = max(2, min(int(deg), len(x)-1))
    coef = np.polyfit(x, y, deg);  yhat = np.polyval(coef, target);  fit = np.polyval(coef, x)
    return yhat, fit
...
if df_long is not None:
    ...
    for industry, row in pv.fillna(0).iterrows():
        ...
        for m in methods:
            if m == "선형추세(OLS)":
                ...
                name = f"{m}({poly_deg}차)"; out_rows.append([industry, name, yh]); fit_store[(industry, name)] = (x, fitted)
            elif m == "CAGR":
    ...
    final_table = pv.copy(); final_table.columns = [f"{c} 실적" for c in final_table.columns]
    ...
    total_2026 = pd.DataFrame({"방법": wide.columns, "구분": "총합", f"{TARGET_YEAR} 합계": [wide[c].sum() for c in wide.columns]})
    ...
    def build_series(industry):
        frames = []
        if industry == "(전체 합계)":
            base = pd.DataFrame({"연도": YEARS, "값": pv.sum(axis=0).values, "시리즈": "실적(합계)"}); frames.append(base)
    ...
    st.altair_chart(alt.Chart(plot_df).mark_line(point=True).encode(x="연도:O", y="값:Q", color="시리즈:N"), use_container_width=True, theme="streamlit")
    ...
    st.altair_chart(alt.Chart(tot_plot).mark_line(point=True).encode(x="연도:O", y="값:Q", color="시리즈:N"), use_container_width=True, theme="streamlit")

    # 다운로드
    st.subheader("다운로드")
    st.download_button("업종별 예측표 CSV 다운로드",
